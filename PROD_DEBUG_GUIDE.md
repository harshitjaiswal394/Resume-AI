...
```powershell
curl -I https://app.jaiswal.shop/api/health
```

### Test CORS Preflight
Verify that the backend handles `OPTIONS` requests from your domain.
```powershell
curl -v -X OPTIONS https://app.jaiswal.shop/api/resume/process `
  -H "Origin: https://app.jaiswal.shop" `
  -H "Access-Control-Request-Method: POST"
```

---

## 📝 Stage 5: Logs & Observability
