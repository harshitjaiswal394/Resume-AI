  ...
  try {
    ...
    if (!body.bullet || !body.role) {
      return NextResponse.json(
        { error: 'Bullet point and role are required' },
        { status: 400 }
      );
    }

    const response = await fetch(`${BACKEND_URL}/api/resume/rewrite`, {
      method: 'POST',
    ...
    if (!response.ok) {
      ...
      return NextResponse.json(
        { error: error.detail || 'Failed to rewrite bullet' },
    ...
    console.error('[API Route] Rewrite error:', error);
    return NextResponse.json(
