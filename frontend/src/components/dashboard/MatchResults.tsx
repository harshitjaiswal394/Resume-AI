    ...
    <div className={`relative bg-white rounded-[32px] p-8 border border-slate-50 shadow-sm transition-all duration-300 ${isLocked ? 'blur-[4px] pointer-events-none opacity-60' : 'hover:shadow-xl hover:-translate-y-1'}`}>
        <div className="flex justify-between items-start mb-6">
          <div className="space-y-1 pr-4">
            ...
            <div className="flex items-center gap-2 text-slate-400 font-bold text-[13px] flex-wrap">
              ...
              <span>{match.company || 'Remote'}</span>
            ...
            {(match.salary_min || match.salary_range) && (
              <div className="flex items-center gap-1 text-emerald-600 font-bold text-xs mt-1">
                ...
                <span>{match.salaryRange || match.salary_range || `${match.salary_min}-${match.salary_max} LPA`}</span>
              </div>
            )}
          </div>
          
          <div className={`
            shrink-0 h-12 w-12 rounded-[18px] flex items-center justify-center font-black text-sm
        ...
        <div className="space-y-4 mb-6">
          ...
          <div className="h-1.5 w-full bg-slate-50 rounded-full overflow-hidden">
            <div 
              className={`h-full ${match.matchScore > 80 ? 'bg-emerald-500' : 'bg-indigo-600'}`}
              style={{ width: `${match.matchScore || match.match_score}%` }}
            />
          </div>
          
          <div className="flex flex-wrap gap-2">
            {(match.matchingSkills || match.matching_skills || ['Python', 'AWS', 'Docker']).map((skill: string) => (
              <Badge key={`m-${skill}`} className="bg-emerald-50 text-emerald-600 border-none px-3 py-1 font-bold text-[11px] rounded-lg">
                ✓ {skill}
              </Badge>
            ))}
            {(match.missingSkills || match.missing_skills || ['Terraform']).map((skill: string) => (
